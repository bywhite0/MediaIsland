...
/// <summary>
/// 音频帧的二进制线格式。音频不走 JSON 信封：线格式是每秒 192000 字节
/// （48kHz × 2 声道 × i16），base64 进 JSON 会再膨胀三分之一，
/// 且每帧都要过一遍序列化器。
///
/// 按每秒字节数立论而不按单帧大小：帧长由采集设备的周期决定，不是协议的一部分
/// （见 docs/medialink-protocol.md「帧长由设备决定，不是固定值」一节），
/// 而每秒的字节数由线格式钉死，不随设备变化。本段的前身写的是「一帧 20ms 约 3840
/// 字节」，实测帧长是约 10ms 的 1920 字节，那句话两个数都已失真——
/// 写死具体帧长的注释就是这么坏掉的。
///
/// 布局（小端）：
/// <code>
