...
- **连续 3 次无回应算失联**，清空整个窗口：留着旧样本会让上层在链路已断时继续按
  ...
  无线链路上是常态，一次丢包就清窗会让 offset 反复进出可用状态。
...
| 预算办不到 | `dMs` 超上界 10000ms；或 `dMs` 减本机设备尾段延迟、再减端点缓冲容量后不足最小抖动缓冲深度（本仓实现 50ms）。为 0、为负同属此类 | 改发送端的 `dMs`，或换更快的本机设备。该往哪边改由方向决定，实现应把方向说出来 |
